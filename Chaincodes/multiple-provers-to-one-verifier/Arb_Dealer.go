...
func (t *Arb_Dealer) Init(stub shim.ChaincodeStubInterface) pb.Response {
	return shim.Success([]byte("Success invoke and not opter!!"))
}

func (t *Arb_Dealer) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
...
func (t *Arb_Dealer) Fait_y_z(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	t0 := time.Now()
	length := len(args)
	...
	for j := 0; j < (m / 2); j++ {
		...
		response := stub.InvokeChaincode("cc_arb_prover", queryArgs, "vegetablefruitchannel") 
		if response.Status != shim.OK {
		...
		if values[0] != "AandS" {
			return shim.Error("The result of getCurState is not unexpected!!!")
		}
		...
		for i := 0; i < 2; i++ {
			countA = countA.Add(as[i].A, EC)
			countS = countS.Add(as[i].S, EC)
			Comms[2*j+i] = as[i].Comm
		}
	}
	mpcrp.Comms = Comms
	mpcrp.A = countA
...
func (t *Arb_Dealer) Fait_x(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	t0 := time.Now()
	length := len(args)
	if (length != 0) && (length != 1) {
		return shim.Error("Incorrect number of arguments. Expecting 0 or 1")
	...
	for j := 0; j < (m / 2); j++ {
		...
		for i := 0; i < 2; i++ {
			countT1 = countT1.Add(t1t2[i].T1, EC)
			countT2 = countT2.Add(t1t2[i].T2, EC)
		}
	}
	mpcrp.T1 = countT1
	mpcrp.T2 = countT2
...
func (t *Arb_Dealer) Gen_Prf(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	t0 := time.Now()
	length := len(args)
	if (length != 0) && (length != 1) {
		return shim.Error("Incorrect number of arguments. Expecting 0 or 1")
	...
	for j := 0; j < (m / 2); j++ {
		...
		for k := 0; k < 2; k++ {
			countThat = new(big.Int).Add(countThat, share[k].That)
			countTaux = new(big.Int).Add(countTaux, share[k].Taux)
			countMu = new(big.Int).Add(countMu, share[k].Mu)
			for i := 0; i < bitsPerValue; i++ {
				left[bitsPerValue*(2*j+k)+i] = share[k].Left[i]
				right[bitsPerValue*(2*j+k)+i] = share[k].Right[i]
			}
		}
	}
...
func (t *Arb_Dealer) Get_MPC_Range_Prf(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	if len(args) != 1 {
...
func (t *Arb_Dealer) Get_Range_History(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	if len(args) != 1 {
...
func main() {
	err1 := shim.Start(new(Arb_Dealer))
	if err1 != nil {
