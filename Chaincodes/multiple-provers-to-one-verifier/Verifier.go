...
func (t *Verifier) Init(stub shim.ChaincodeStubInterface) pb.Response {
...
func (t *Verifier) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
...
func (t *Verifier) Init_Setup(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	t0 := time.Now()
	...
	// args: Veclength prover xxx  xxx  dealer xxx
	length := len(args)
	VecLength, _ := strconv.Atoi(args[0])
...
func (t *Verifier) Get_Setup_Info(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	if len(args) != 1 {
...
func (t *Verifier) Get_KeyID_Sess_Num(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	if len(args) != 0 {
...
//Only the user or admin of verfier can verify proof
func (t *Verifier) Ver_Prf(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	t0 := time.Now()
	length := len(args)
	...
	response := stub.InvokeChaincode("cc_dealer", queryArgs, "vegetablefruitchannel")
